...
export function ScientistItem({ item }) {
  ...
  function hexToDec(hexValue) {
    if (hexValue) {
      return new BN(hexValue.substring(2), 16).toString();
    }
    return "";
  }
    ...
    <>
      <ListItem className="border-b border-gray-200 py-4 items-center">
        ...
        <Button
          onClick={handleOpen}
          variant="text"
          className="ml-auto p-2 text-blue-500 hover:bg-blue-50"
        >
      ...
      <Dialog open={open} handler={handleOpen} size="lg" className="p-4">
        <Card className="w-full">
          <CardHeader
            floated={false}
            shadow={false}
            className="bg-blue-100 flex justify-between items-center p-4"
          >
          ...
          <CardBody className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
            <div className="flex flex-col gap-2">
              ...
              <Typography>{data?.scientist_add}</Typography>
              ...
              <Typography>{data?.image_VR?.length}</Typography>
            </div>
